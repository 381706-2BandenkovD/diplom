...
a_r = 0.01, b_r = 0.01, R0 = 2, R1 = 1, q_r = 5.604, k_r = 0.1;

size_t t = 200000;
double t0 = 0, h = 0.05, x0 = 0.2, y0_ = 0.2, z0_ = 0.2;
double i_py = 0.0, i_end = 0.014;
...
void RKutt(std::vector<double> &a, std::vector<double>&b,double x0, double y0, double z0, size_t t, double h, double time0, double w_q, size_t &iter) {
  ...
  for (t0; t0 < t; t0 += h) {
    ...
    double k2_y = fy(x[i] + k1_y * 0.5, y[i] + k1_y * 0.5, t0, w_q);
    double k3_y = fy(x[i] + k2_y * 0.5, y[i] + k2_y * 0.5, t0, w_q);
    double k4_y = fy(x[i] + k3_y, y[i] + k3_y, t0, w_q);
    yt = y[i] + h * (k1_y + 2 * k2_y + 2 * k3_y + k4_y) / 6;
    y[i + 1] = yt;

    double k1_x = fx(x[i], y[i], t0, w_q);
    double k2_x = fx(x[i] + k1_x * 0.5, y[i] + k1_x * 0.5, t0, w_q);
    double k3_x = fx(x[i] + k2_x * 0.5, y[i] + k2_x * 0.5, t0, w_q);
    double k4_x = fx(x[i] + k3_x, y[i] + k3_x, t0, w_q);
    xt = x[i] + h * (k1_x + 2 * k2_x + 2 * k3_x + k4_x) / 6;
    x[i + 1] = xt;

    double k1_z = fz(x[i], z[i], t0, w_q);
    double k2_z = fz(x[i] + k1_z * 0.5, z[i] + k1_z * 0.5, t0, w_q);
    double k3_z = fz(x[i] + k2_z * 0.5, z[i] + k2_z * 0.5, t0, w_q);
...
BOOST_PYTHON_MODULE(threeSystem) {
